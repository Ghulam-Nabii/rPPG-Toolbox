...
def preprocess_raw_video(videoFilePath,start_time, dim1=240,dim2=320,normalize = False):
    ...
    Xsub = np.zeros((totalFrames, dim2, dim1, 3),dtype = np.float32)
    ...
    while success:
        ...
        img = cv2.rotate(img, cv2.ROTATE_180) # rotate 90 degree
    ...
    # plt.imshow(Xsub[0])
    # plt.title('Sample Preprocessed Frame')
    # plt.savefig('/data/rPPG_dataset/preprocess/result/2.png')
    # plt.show()
    #########################################################################
    ...
    if normalize == True:
        ...
        # dXsub = np.zeros((normalized_len, dim2, dim1, 3), dtype = np.float32)
